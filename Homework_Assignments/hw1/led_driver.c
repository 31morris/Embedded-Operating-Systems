...
const int eight_led_pins[8] = {GPIO_24, GPIO_23, GPIO_8, GPIO_7, GPIO_12, GPIO_16, GPIO_20, GPIO_21};
/* 
...
static ssize_t etx_write(struct file *filp, const char __user *buf, size_t len, loff_t *off)
{
    ...
    int total_dis = 0; // 總共要亮幾顆LED
    if (copy_from_user(rec_buf, buf, len) > 0) {
        pr_err("ERROR: Not all the bytes have been copied from user\n");
    }

    total_dis = rec_buf[0] - '0'; // 把收到的字元轉成整數
    pr_info("Write Function: total_dis = %d\n", total_dis);

    if (rec_buf[0] >= '0' && rec_buf[0] <= '9') {
        // 1. 一開始先亮 total_dis 顆LED
        for (int i = 0; i < total_dis; i++) {
            gpio_set_value(eight_led_pins[i], 1); // 先全部打開
        }

        // 2. 每秒關掉一顆
        for (int i = total_dis - 1; i >= 0; i--) {
            msleep(1000); // 等1秒
            gpio_set_value(eight_led_pins[i], 0); // 關掉最右邊的一顆
        }
    } else {
...
{ 
  ...
  if((alloc_chrdev_region(&dev, 0, 1, "Distance_Dev")) <0){ 
  ...
  for(int i = 0; i < 8; i++)
  {
    if(gpio_request(eight_led_pins[i], "eight_led_pins") < 0)
    {
  ...
  */ 
  ...
  for(int i = 0; i < 8; i++)
  {
    gpio_export(eight_led_pins[i], false);
  }
...
{ 
  for(int i = 0; i < 8; i++)
  {
    gpio_unexport(eight_led_pins[i]);
    gpio_free(eight_led_pins[i]);
  }
